...
class LayerNormalization(LightningModule):
    def __init__(self, eps: float = 1e-6) -> None:
        super().__init__()
        self.eps = eps
        self.alpha = nn.Parameter(torch.ones(1))    # alpha and bias both will be learned along the way 
...
class PositionalEncoding(LightningModule):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # create matrix of shape (seq_len, d_model) zeros
        ...
        # register to buffer so that it remains after model.to(device)
...
class MultiHeadAttentionBlock(LightningModule):
    ...
    def attention(
        ...
        # attention formula from the foundation paper
        attention_scores = (query @ key.transpose(-2, -1)) / math.sqrt(d_k)

        # filling the masked positions with -1e9 (negative infinity) in the place of zeros
        if mask is not None:
...
class Decoder(LightningModule):
    ...
    def forward(
        self, x: torch.Tensor, encoder_output: torch.Tensor, src_mask, tgt_mask) -> torch.Tensor:
        for layer in self.layers:
    ...
    for _ in range(N):
        ...
        decoder_blocks.append(
            DecoderBlock(
                ...
                decoder_cross_attention_block, #coming from encoder
                feed_forward_block,
                dropout,
            )
        )
