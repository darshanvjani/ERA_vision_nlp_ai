...
class Net(nn.Module):
    def __init__(self, dropout_value = 0.01):
        ...
        self.convblock1 = nn.Sequential(
            ...
            nn.Conv2d(in_channels = 16, out_channels = 32, kernel_size = 3, padding = 2, bias = False, dilation = 2), # 32 -> 32 | 3 -> 7 | 1 -> 1
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Dropout(dropout_value),
            
            
            nn.Conv2d(in_channels = 32, out_channels = 16, kernel_size = 3, padding = 1, bias = False, stride = 2) # 32 -> 16 | 7 -> 9 | 1 -> 2
        ...
        self.convblock2 = nn.Sequential(
            ...
            nn.Conv2d(in_channels = 64, out_channels = 32, kernel_size = 3, padding = 1, bias = False, stride = 2) # 16 -> 8 | 17 -> 21 | 2 -> 4
            )
        
        self.convblock3 = nn.Sequential(
            nn.Conv2d(in_channels = 32, out_channels = 64, kernel_size = 3, padding = 1, bias = False),  # 8 -> 8 | 21 -> 29 | 4 -> 4
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Dropout(dropout_value),
            
            
            nn.Conv2d(in_channels = 64, out_channels = 120, kernel_size = 3, padding = 1, bias = False), # 8 -> 8 | 29 -> 37 | 4 -> 4
            nn.ReLU(),
            nn.BatchNorm2d(120),
            nn.Dropout(dropout_value),
            
            
            nn.Conv2d(in_channels = 120, out_channels = 64, kernel_size = 3, padding = 1, bias = False, stride = 2) # 8 -> 4 | 37 -> 45 | 4 -> 8
            )
        
        self.convblock4 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # 4 -> 1 | 45 -> 69 | 8 -> 8
