...
class ScalePrediction(pl.LightningModule):
    def __init__(self, in_channels, num_classes):
        super().__init__()
        self.pred = nn.Sequantial(
            CNNBlock(in_channels, 2 * in_channels, kernel_size=3, padding=1),
            CNNBlock(2 * in_channels, (num_classes + 5) * 3, kernal_size=1, bn_act=False)
        )
    ...
    def _create_conv_layers(self):
        ...
        for module in model_config:
            if isinstance(module, tuple):
                out_channels, kernal_size, stride = module
                layers.append(
                    CNNBlock(in_channels, out_channels, kernal_size=kernal_size, stride=stride, padding=1 if kernal_size==3 else 0)
                )
                ...
                if module == "S":
                    layers += [
                        ...
                        CNNBlock(in_channels, in_channels//2, kernal_size=1),
